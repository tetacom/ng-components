...
@Component({
  ...
  imports: [CommonModule, NgtArgs, NgtsText, NgtPush],
  // schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
export class Block3dComponent<
  T extends Block3dPoint,
> extends Base3dSeriesComponent<T> {
  ...
  setPoints() {
    if (this.series && this.scales) {
      this.blocks = this.series?.data?.map(_ => {
