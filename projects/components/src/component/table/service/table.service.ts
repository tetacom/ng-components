...
export class TableService<T> {
  ...
  private _dict: BehaviorSubject<IDictionary<IIdName<any>[]>> = new BehaviorSubject<
    IDictionary<IIdName<any>[]>
  >({});
  private _filterOptions: BehaviorSubject<IDictionary<IIdName<any>[]>> = new BehaviorSubject<
    IDictionary<IIdName<any>[]>
  >({});
  private _state: BehaviorSubject<FilterState> = new BehaviorSubject<FilterState>(
    new FilterState()
  );
  ...
  allRowsSelected(): boolean | null {
    if (
      ...
      this._displayData.value.every(
        (_) => this._selectedRows.value.findIndex((selected) => selected === _.data) >= 0
      )
    ) {
      return true;
