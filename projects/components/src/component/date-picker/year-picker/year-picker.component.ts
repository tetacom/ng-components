...
export class YearPickerComponent implements OnInit {
  ...
  isDisabled(year: number) {
    if (this.minMax) {
      const isSuitableMax =
        dayjs(new Date().setFullYear(year)).startOf('year').isAfter(new Date(this.minMax.max)) &&
        this.minMax.max !== null;
      const isSuitableMin =
        dayjs(new Date().setFullYear(year)).endOf('year').isBefore(new Date(this.minMax.min)) &&
