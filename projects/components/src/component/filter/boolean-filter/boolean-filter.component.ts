...
export class BooleanFilterComponent<T> extends FilterComponentBase<T> implements OnInit {
  ...
  @Output() filterChanged: EventEmitter<FilterBase<boolean>> = new EventEmitter<FilterBase<boolean>>();
  ...
  private getFilter(): BooleanFilter {
    let filter = this.state.booleanFilters.find((f) => f.field === this.column.filterField);
    if (!filter) {
