...
export class TestChartComponent {
  ...
  getConfig(): IChartConfig {
    ...
    const cs = this.getCasingSeries(construction.casing.sort((a, b) => a.intervalTopMd - b.intervalTopMd), traRad)
    return {
      tooltip: {
  ...
  getPoint(point: {
    ...
    const zenithAngle = prev.zenithAngle + (next.zenithAngle - prev.zenithAngle) * (point.md - prev.measuredDepth) / (next.measuredDepth - prev.measuredDepth)
    const azimuthAngle = prev.azimuthAngle + (next.azimuthAngle - prev.azimuthAngle) * (point.md - prev.measuredDepth) / (next.measuredDepth - prev.measuredDepth)
    ...
    const x = prev.coordX + (point.md - prev.measuredDepth) * (Math.sin(prev.zenithAngle) * Math.sin(prev.azimuthAngle) + Math.sin(zenithAngle) * Math.sin(azimuthAngle)) * Math.tan(beta / 2) / beta;
    const y = prev.coordY + (point.md - prev.measuredDepth) * (Math.sin(prev.zenithAngle) * Math.cos(prev.azimuthAngle) + Math.sin(zenithAngle) * Math.cos(azimuthAngle)) * Math.tan(beta / 2) / beta;
    const z = prev.coordZ + (point.md - prev.measuredDepth) * (Math.cos(prev.zenithAngle) + Math.cos(zenithAngle)) * Math.tan(beta / 2) / beta;
